...
export const compileRawToolDef = {
  ...
  description:
    ...
    `wiki article in ${config.dirs.wiki}/. Use this to grow the wiki from ingested sources.`,
  inputSchema: {
    type: "object" as const,
    properties: {
      raw_path: {
        type: "string",
        description: `Path to the raw source document (in ${config.dirs.raw}/)`,
      },
      title: {
        type: "string",
      ...
      directory: {
        type: "string",
        description:
          `Target subdirectory within ${config.dirs.wiki}/ (e.g., 'books-and-resources' or 'tech/ai'). ` +
          `If omitted, the article is created directly in ${config.dirs.wiki}/.`,
      },
    },
...
export const updateIndexToolDef = {
  ...
  description:
    `Update an index file in ${config.dirs.indexes}/. Use this to maintain master-index.md, ` +
  ...
  inputSchema: {
    ...
    properties: {
      index_path: {
        type: "string",
        description: `Relative path to the index file (e.g., '${config.dirs.indexes}/master-index.md')`,
      },
      section: {
        type: "string",
